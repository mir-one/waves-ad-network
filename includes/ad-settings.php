...
if(isset($_POST['waves_setting_submit']) && current_user_can('administrator')){

  if ( ! isset( $_POST['waves-network-settings-form-field'] ) || ! wp_verify_nonce( $_POST['waves-network-settings-form-field'], 'waves-network-settings-form-action' ) ) {
...
<form action='' method="post">
  <?php wp_nonce_field( 'waves-network-settings-form-action', 'waves-network-settings-form-field'); ?>
	<table class="form-table">
		<tbody>
			<tr>
				<th scope="row">
          <label class="tooltip" for="label">Set API Server
            <span class="tooltiptext">Set any node on the Waves from list: https://wavesexplorer.com/nodes</span>
          </label>
        </th>
			...
			<tr>
				<th scope="row"><label class="tooltip"  for="label">Minimum Amount in WAVES  <span class="tooltiptext">Minimum payment required for an Ad to be displayed, in WAVES</span></label></th>
				<td><input name="min_amount" placeholder="" type="text" value="<?php _e(intval(get_option('min_amount')));?>" class="regular-text" required></td>
			</tr>
			<tr>
				<th scope="row"><label class="tooltip"  for="address">Ad Display Cost / Impressions in WAVES<span class="tooltiptext">Set the cost, in WAVES, per amount of Ad impressions displayed, for all Ad Segments, multiple ads will round-robin</span></label> </th>
				<td><input name="ad_cost" placeholder="" type="text" value="<?php _e(intval(get_option('ad_cost')));?>" class="regular-text" required> WAVES Per <input name="ad_time" placeholder="" type="text" value="<?php _e(intval( get_option('ad_time')));?>" class="regular-text" required> Impressions</td>
			</tr>
			<tr>
				...
				<td><textarea name="blacklist" style="width: 90%;height: 150px"><?php if( esc_html(get_option('blacklist')) == ''){echo "Enter Words, Expressions, Address with comma separated values to block each Words, Address & Expressions. For example : blue,X+5,3PA8GR3PLpGmwYhSdQ4TSK7sGnjeC2XQQS2";}else{ echo esc_html(get_option('blacklist'));} ;?></textarea></td>
			</tr>
			<tr>
      ...
      <tr>
        ...
        <td>
              ...
              <ul>
                ...
                <li><b>Text Ad Example:</b> Ad (Waves Ad Network)(Ad Network using the Waves blockchain)(https://t.me/wavesadnetwork)</li>
                <li><b>Text Ad submission process:</b> Send a transaction on the Waves, with an attachment in the above format, to an assigned address. An assigned address is configured/linked to an Ad Segment.</li>
	...
	<p class="submit">
		<input type="submit" name="waves_setting_submit" class="button button-primary" value="Save">
	</p>
</form>