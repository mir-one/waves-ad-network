<?php
/**
 *	@package Waves ads network 
 */
/*
Plugin Name: Waves Ad Network
Plugin URI: https://t.me/wavesadnetwork
Description: Peer-to-Peer Ad Network using the Waves blockchain
Version: 1.0.0
Auther: Roman Inozemtsev | Refactoring from TAN https://t.me/gordobtel
Auther URI: https://t.me/inozemtsev_roman
Text Domain: waves-ad-network
License: GPLv3
*/

namespace WavesAdsNetwork;
...
global $wpdb;
$wadn_jal_db_version = '1.0';

class WADN_WavesAdsNetwork
{
	...
	*/
	function __construct(){
		...
		*/
		$this->wadn_create_WalletAdress_table();
		$this->wadn_create_adSegment_table();
		$this->wadn_create_adSize_table();
		$this->wadn_create_ad_segment_details_table();
		$this->wadn_create_min_amount_txid_table();
		...
		*/
		add_action('admin_post_submit-form-waves', array($this , 'wadn_handle_form_action')); 
		add_action('admin_post_nopriv_submit-form-waves', array($this, 'wadn_handle_form_action')); 

		add_action('admin_post_submit-form-add-ad-slots', array($this , 'wadn_handle_form_action_slot')); 
		add_action('admin_post_nopriv_submit-form-add-ad-slots', array($this, 'wadn_handle_form_action_slot')); 

		
		add_action('wan_cronjob', array($this, 'wadn_do_this_hourly'));

		add_filter( 'cron_schedules', array($this,'wadn_add_cron_interval' ));
	...
	function wadn_add_cron_interval( $schedules ) {
	...
	function wadn_activate(){
		
		if (! wp_next_scheduled ( 'wan_cronjob' )) {
			wp_schedule_event(time(), 'onemin', 'wan_cronjob');
			update_option('api_server','https://nodes.wavesplatform.com');
		}
	}

	function wadn_deactivate(){

		// nothing here
		wp_clear_scheduled_hook('wan_cronjob');

	}
	function wadn_do_this_hourly() {
	...
	*/
	function wadn_create_WalletAdress_table(){

		global $wpdb;
		global $wadn_jal_db_version;
		...
		add_option( 'wadn_jal_db_version', $wadn_jal_db_version );
	...
	*/
	function wadn_create_adSegment_table(){
	...
	*/
	function wadn_create_adSize_table(){
	...
	** 
	*/
	function wadn_create_ad_segment_details_table(){
	...
	** 
	*/
	function wadn_create_min_amount_txid_table(){
	...
	*/
	...
	function wadn_handle_form_action(){
		if(!current_user_can('administrator') || ! isset( $_POST['waves-network-walletAddress-form-field'] ) || ! wp_verify_nonce( $_POST['waves-network-walletAddress-form-field'], 'waves-network-walletAddress-form-action' ) ) {
	...
	*/
	...
	function wadn_handle_form_action_slot(){

		if(!current_user_can('administrator') || ! isset( $_POST['waves-network-slot-form-field'] ) || ! wp_verify_nonce( $_POST['waves-network-slot-form-field'], 'waves-network-slot-form-action' ) ) {
...
if( class_exists('\WavesAdsNetwork\WADN_WavesAdsNetwork') ){
	$waves = new \WavesAdsNetwork\WADN_WavesAdsNetwork();
}
...
foreach($datas as $data){
	...
	$cb = function() use ($slug_name) {
		...
		if(!empty($datas)){
			foreach($datas as $data){
				...
				if($width == 728){
					$img_width = '2%';
					$style_a = 'font-size:13px;line-height: 1.8em;';
				}else if($width = 320){
					$img_width = '4%';
					$style_a = 'font-size:13px;line-height: 2em;';
				}else{
					$img_width = '5%';
				}

				$a[] = '<div style="width:'.$width.'px;height:'.$height.'px;border:1px solid black;padding: 10px;text-align: center;'.$style_a.'"><b style="text-transform: uppercase;">'.$headline.'</b><br>'.$des.'<br><a href="'.$clickable.'" target="_blank">'.$clickable.'</a><a href="https://wavesexplorer.com/tx/'.$txid.'" target="_blank" style="margin-left:10px;"><img src="'.$imgUrl.'" alt="transaction id" width="'.$img_width.'"></a></div>wan_idssx:'.$ids;
				$x++;
    ...
    add_shortcode( "WAN-$slug_name", $cb );
}

// activation
register_activation_hook( __FILE__, array( $waves, 'wadn_activate' ) );

// deactivation
register_deactivation_hook( __FILE__, array( $waves, 'wadn_deactivate' ) );